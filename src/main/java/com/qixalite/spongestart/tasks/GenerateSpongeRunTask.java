...
public abstract class GenerateSpongeRunTask extends GenerateRunTask {
    ...
    public void refresh() {
        ...
        if (!getProject().getPlugins().hasPlugin("net.minecrell.vanillagradle.server")) {
            String outputFolder = new File(getProject().getRootDir(), (getExtension().getIdeaOutput() == null ? "out" : getExtension().getIdeaOutput()) + File.separatorChar + "production").getAbsolutePath();
                ...
                configuration.getDependencies().forEach(dependency -> {
                    if (dependency instanceof ProjectDependency) {
                        s.append(';')
                                .append(outputFolder)
                                .append(File.separatorChar)
                                .append(((ProjectDependency) dependency).getDependencyProject().getExtensions().getByType(IdeaModel.class).getModule().getName())
                                .append("_main");
                    }
                });
            });
